...
class Empirical():
    def __init__(self, args, config):
        ...
        self.config.attack.rand_type = self.config.purification.rand_type
    ...
    def run(self, log_progress):
        ...
        if self.config.structure.dataset in ["MNIST", "FashionMNIST"]:
            ...
            optimizer = optim.Adam(network_clf.parameters(), lr=0.001, weight_decay=0., betas=(0.9, 0.999), amsgrad=False)
        elif self.config.structure.dataset in ["CIFAR10", "CIFAR10-C", "CIFAR100"]: # CIFAR10 setting, trained by WideResNet
