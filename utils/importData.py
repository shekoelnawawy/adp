...
def importData(dataset, train, shuffle, bsize):
    ...
    '''
    # Set transform
    dataset_list = ["MNIST", "CIFAR10", "FashionMNIST", "CIFAR10-C", "CIFAR100", "TinyImageNet"]
    ...
    if dataset=="MNIST":
        ...
        if train:
            dataset = TinyImageNet.TinyImageNet(os.path.join(path_root, 'datasets', 'tiny-imagenet-200', 'train'), train=train)
        else:
            dataset = TinyImageNet.TinyImageNet(os.path.join(path_root, 'datasets', 'tiny-imagenet-200', 'val'), train=train)
